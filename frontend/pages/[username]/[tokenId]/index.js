...
function AToken({ token, tokenMetadata, auctionResult }) {
  ...
  const { auction, activities } = token;
      ...
      {user?._id != auction?.createdBy?._id && displayTime !== AUCTION_ENDED && (
        <div className="border-t border-gray-300 px-8 py-8 grid">
          <Button
            ...
            onClick={() =>
              router.push(`/${auction?.createdBy?.username}/${token?._id}/bid`)
            }
          />
        </div>
      )}
            ...
            <div className="sm:grid-cols-120px sm:gap-8 grid items-center grid-cols-80px gap-4">
              ...
              <div className="">
                ...
                <div className="flex font-semibold text-2xl">
                  <Link href={`/${token?.user?.username}`}>
                    <div className="text-brand">@{token?.user?.username}</div>
                  </Link>
                </div>
              </div>
            </div>
...
export async function getServerSideProps({ params }) {
  try {
    ...
    // Token
    const { data: token } = await TokenService.fetchToken(
      params.username,
      params.tokenId
    );
    ...
    const isAuctionEnded =
      moment(moment(token?.auction?.biddingEndDate)).valueOf() <
      new Date().getTime();
