...
export const SearchResult = ({
  ...
  if (contentType?.includes("video")) {
    file = (
      <Video
        url={imageUrl}
        className={`m-0 min-w-2rem max-w-2rem min-h-2rem max-h-2rem lg:min-h-3.125 lg:min-w-3.125 lg:max-h-3.125 lg:max-w-3.125 bg-gray-200 bg-cover object-cover bg-center ${
          token ? "rounded-0.625" : "rounded-full"
        }`}
      />
    );
