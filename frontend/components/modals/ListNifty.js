...
import {
  ...
  CHAINID_ERROR,
  REQUEST_LISTING,
  NIFTY_LISTED_SUCCESS,
  REQUEST_LISTING_ERROR,
} from "../../constants";

const web3 = new Web3Instance();

export const ListNifty = ({ modal, showModal }) => {
  const { payload: nifty } = modal;
  ...
  const submitHandler = async (values) => {
    try {
      ...
      const tokenId = nifty.chainInfo.id;
      const biddingEndTimeStamp = moment.unix(biddingEndDate).unix();
      const data = await web3.listNifty(
        tokenId,
        biddingEndTimeStamp,
        ETHAccount
      );
      ...
      const auctionData = {
        chainInfo: { ...nifty.chainInfo, txId: data.transactionHash, listId },
        niftyId: nifty._id,
      ...
      toast.success(<SuccessMsg msg={NIFTY_LISTED_SUCCESS} />);

      // Redirect to nifty page
      router.push(`/${user.username}/${nifty._id}`);
    } catch (err) {
      console.log(err);
