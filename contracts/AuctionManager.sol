...
contract AuctionManager is Ownable {
    ...
    event FundTransfered(address indexed _to, uint indexed _amount, address indexed by);
    ...
    constructor(address _nftTokenContractAddress, uint _commissionRate) {
        // Checks
    ...
    function listItem(uint _tokenId, uint _biddingEndTimestamp) public notPaused {
        // Checks
        address owner = NFTToken(nftTokenContractAddress).ownerOf(_tokenId);
    ...
    function settleAuction(uint _listId) public notPaused {
        ...
        NFTToken(nftTokenContractAddress).safeTransferFrom(item.owner, item.highestBidder, item.tokenId);
    ...
    function getNiftyTokenContractAddress() public view onlyOwner returns(address) {
        return nftTokenContractAddress;
    }

    function setNiftyTokenContractAddress(address _nftTokenContractAddress) public onlyOwner {       
        nftTokenContractAddress = _nftTokenContractAddress;
    }
    ...
    function withdrawFund(uint _amount, address _to) public onlyOwner {
        // Checks
        require(_amount > 0, 'Amount can not be zero');
        require(_to != address(0), 'Need a valid payee address');
        require(_to != address(this), 'Can not withdraw to contract address');
        require(address(this).balance >= _amount, 'Contract does not have enough balance');
        require(address(this).balance - commission >= _amount, 'Contract does not have enough balance');

        // Effects
        uint amount = address(this).balance - commission;
        address payable to = payable(_to);

        emit FundTransfered(_to, amount, getOwner());
